package me.lukavns.commandframework.bukkit.bootstrap;
...
import me.lukavns.commandframework.bukkit.internal.BukkitExceptionMessenger;
import me.lukavns.commandframework.bukkit.registration.BukkitCommandRegistrar;
import me.lukavns.commandframework.bukkit.resolve.BukkitArgumentResolvers;
...
public final class BukkitCommandFramework {
    ...
    private BukkitCommandFramework(
        Plugin plugin,
        ...
        BukkitArgumentResolvers.registerDefaults(this.resolverRegistry);
        ...
        BukkitSuggestionAdapter adapter = new BukkitSuggestionAdapter(
            this.plugin,
            this.dispatcher,
            messageResolver == null ? new BukkitExceptionMessenger(this.messageHolder) : messageResolver
        );
        this.registrar = new BukkitCommandRegistrar(this.plugin, commandRegistry, adapter);
    }
    ...
    public BukkitCommandRegistrar commandRegistrar() {
    ...
    private void registerProvidedValues() {
        this.providedValueRegistry.register(Plugin.class, new ProvidedValueFactory<Plugin>() {
            ...
            public Plugin provide(CommandContext<?> context) {
                return BukkitCommandFramework.this.plugin;
            }
        });
        this.providedValueRegistry.register(Server.class, new ProvidedValueFactory<Server>() {
            @Override
            public Class<Server> type() {
                return Server.class;
            }

            @Override
            public Server provide(CommandContext<?> context) {
                return BukkitCommandFramework.this.plugin.getServer();
            }
        });
    ...
    private void registerRuntimePluginType() {
        this.providedValueRegistry.register((Class<Plugin>) this.plugin.getClass(), new ProvidedValueFactory<Plugin>() {
            @Override
            public Class<Plugin> type() {
                return (Class<Plugin>) BukkitCommandFramework.this.plugin.getClass();
            }
    ...
    public static final class Builder {
        ...
        public BukkitCommandFramework build() {
            return new BukkitCommandFramework(this.plugin, this.asyncExecutor, this.messageResolver);
        }
    }
}
